  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-10 space-y-8">
      <div className="text-center">
        <p className="text-xl">{quizIntro}</p>
      </div>
      ...
      >
        <div className="relative z-10">
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-8 shadow-lg">
            <h3 className="text-3xl font-bold mb-8 text-black">{currentQ.prompt}</h3>
            <div className="space-y-4">
              {currentQ.options.map((opt, i) => {
                const id = `${currentQ.id}_${i}`;
                return (
                  <label key={id} className="flex items-start gap-4 cursor-pointer p-4 rounded-lg hover:bg-gray-100 transition-colors">
                    <input
                      type="radio"
                      name={currentQ.id}
                      ...
                      className="mt-1"
                    />
                    <span className="text-lg text-black">{opt.label}</span>
                  </label>
                );
              })}
            </div>
      ...
      <div className="flex justify-between items-center">
        <button
          ...
          className="px-6 py-3 text-gray-600 hover:text-black disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Back
        </button>
