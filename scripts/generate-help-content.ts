/**
 * Generates help-center/content.json and the three collection markdown files.
...
type Item = { slug: string; title: string; summary: string; order: number; body?: string };

function commonBody(focus: string, shot = "main workspace") {
  return `## Overview
...
function mdBlock(collection: string, sub: string | undefined, a: Item) {
...
function series(
  ...
  prefix: string,
  titles: [string, string][], // [slugSuffix, title] — summary auto
  startOrder: number
): Item[] {
...
const scheduleSubs: Record<string, Item[]> = {
...
scheduleSubs["social-profile-manager"][0].slug = "how-to-connect-instagram";

const linkInBioSubs: Record<string, Item[]> = {
...
const scheduleTop: Item[] = series("schedule-and-publishing", undefined, "sched-top", [
...
function countArticles(col: (typeof manifest.collections)[0]): number {
  const top = col.articles.length;
  const sub = col.subCollections.reduce((n, s) => n + s.articles.length, 0);
  return top + sub;
}
...
if (total !== 74) {
  ...
  console.warn(`Expected 74 articles, got ${total}`);
}
...
for (const sub of manifest.collections[0].subCollections) {
  const items = gettingStartedSubs[sub.slug as keyof typeof gettingStartedSubs];
...
Regenerate markdown + manifest from \`scripts/generate-help-content.ts\` if you bulk-edit structure.
`,
  "utf8"
);

// eslint-disable-next-line no-console
console.log(`Wrote help-center (${total} articles).`);
